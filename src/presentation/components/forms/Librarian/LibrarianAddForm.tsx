...
import { useLibrarianAddFormController } from "./LibrarianAddForm.controller";
 ...
 */
export const LibrarianAddForm = (props: { onSubmit?: () => void, id?:string }) => {
    const { formatMessage } = useIntl();
    const { state, actions, computed } = useLibrarianAddFormController(props.onSubmit, props.id); // Use the controller.
                ...
                <Grid container item direction="row" xs={12} columnSpacing={4}>
                    ...
                    <Grid container item direction="column" xs={6} md={6}>
                        ...
                        > {/* Wrap the input into a form control and use the errors to show the input invalid if needed. */}
                            ...
                            </FormLabel> {/* Add a form label to email what the input means. */}
                            <OutlinedInput
                    ...
                    <Grid container item direction="column" xs={6} md={6}>
                        ...
                        >
                            <FormLabel>
                                <FormattedMessage id="globals.librarianPosition" />
                            </FormLabel>
                            <OutlinedInput
                                {...actions.register("position")}
                                placeholder={formatMessage(
                                    { id: "globals.placeholders.textInput" },
                                    {
                                        fieldName: formatMessage({
                                            id: "globals.librarianPosition",
                                        }),
                                    })}
                                autoComplete="none"
                            />
                            <FormHelperText
                                hidden={isUndefined(state.errors.position)}
                            >
                                {state.errors.position?.message}
                            </FormHelperText>
                        </FormControl>
                    </Grid>
